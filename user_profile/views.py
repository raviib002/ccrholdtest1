...
from user_profile.forms import ( CustomPasswordChangeForm,RegistrationStep3,RegistrationStep2)
from django.contrib.auth import views as auth_views
...
def registration_profile_info(request):
    if request.method == "GET":
        ...
        form = RegistrationStep2(request.POST)
        if form.is_valid():
            ...
            if not User.objects.filter(email=request.POST.get('email')) and not  AdditionalProfile.objects.filter(mobile_no=request.POST.get('mobile_number')) .exists():
                ...
                AdditionalProfile.objects.create(user=user_id,
                ...
                if settings.SEND_MAIL_ALL_PLACE:
                    ...
                    html_content = "Welcome to CCRH Portal Your Password {random_string} and Email {email}" .format(email=user_id.email,
